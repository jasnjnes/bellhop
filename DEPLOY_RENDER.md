# Deploy the GitHub Project Gateway on Render

This is a one-time setup. After it is connected, use a normal Claude chat on desktop or
mobile and explicitly say:

> Use the GitHub Project Gateway to create a private repository named `my-project`.

GitHub stores all project files and versions. Render only runs the stateless MCP gateway.

## 1. Put this code in GitHub

You need one repository for the gateway itself.

### Browser-only method

1. Extract `render-github-mcp-gateway.zip`.
2. In GitHub, create a new **private** repository named `render-github-mcp-gateway`.
3. Do not add a README, `.gitignore`, or license.
4. Open the empty repository.
5. Select **Add file → Upload files**.
6. Drag the **contents inside** the extracted `render-github-mcp-gateway` folder into
   GitHub.
...
```bash
unzip render-github-mcp-gateway.zip
cd render-github-mcp-gateway

git init
git add .
git commit -m "Initial Render GitHub MCP gateway"
git branch -M main
git remote add origin https://github.com/YOUR_GITHUB_NAME/render-github-mcp-gateway.git
...
4. Select the private `render-github-mcp-gateway` repository.
...
```text
https://github-project-gateway.onrender.com
```
...
```text
OAuth Client ID: claude-render-github-gateway
...
> Use the GitHub Project Gateway and run `github_connection_status`.

Then:

> Use the GitHub Project Gateway and list my five most recently updated repositories.

Then create a disposable test repository:

> Use the GitHub Project Gateway to create a private repository named
...
> Use the GitHub Project Gateway for this project. Create a new private repository called
...
> Use the GitHub Project Gateway and work in `YOUR_NAME/REPOSITORY`. Read the current
...
### A commit was rejected with `conflicting_paths`

This is the concurrency guard doing its job, not a bug. The branch changed since
Claude last read it, and your new edit touches one of the same files, so the
gateway refused to overwrite the newer work. Ask Claude to re-read the listed
files at the current head and commit again. Edits to *different* files are
re-applied automatically (the result shows `rebased_onto_current_head: true`),
so you will only see this when two changes genuinely overlap.

### "Too many attempts" on the authorization page

The login form locks after several wrong passwords (default 5 within 15 minutes)
and returns for a few minutes before accepting attempts again. Wait for the
window to clear, then enter the correct `MCP_LOGIN_PASSWORD`. The limit resets on
a successful login. Because this protection is in-memory, a Render redeploy or
restart also clears it.

## Security notes

- The GitHub token on Render is the whole blast radius. Prefer issuing it from a
  dedicated GitHub account used only for these document repos, so a leak cannot
  touch your primary account. Add your main account as a collaborator on
  individual repos when you want to pull one into Claude Code.
- Access tokens live one hour; refresh tokens live seven days. A captured token
  therefore ages out on its own within a week. Tune `ACCESS_TOKEN_TTL_SECONDS` /
  `REFRESH_TOKEN_TTL_SECONDS` only if you understand the trade-off.
- Use a long, random `MCP_LOGIN_PASSWORD` (the `secrets.token_urlsafe(48)` value
  from setup). The attempt limiter blunts guessing but is not a substitute for
  real entropy.
